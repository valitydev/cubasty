...
handle_command({ok, {Args, ["setup"]}}) ->
    ...
    case maps:take(database, Opts) of
        ...
        {Database, Opts1} ->
            case with_connection(Opts1#{database => "postgres"}, create_db_fun(Database)) of
                ok -> handle_command({ok, {Args, ["run"]}});
                Other -> handle_command_result(Other)
            end
    end;
...
with_connection(Args, Fun) ->
    case open_connection(Args) of
        {ok, Conn} ->
            Fun(Conn);
        {error, Error} ->
            Msg = io_lib:format("Failed to connect to database: ~p~n", [Args]),
            {error, lists:flatten(Msg), [Error]}
    end.

create_db_fun(Database) ->
    fun(Conn) -> create_db_if_not_exists(Conn, Database) end.

create_db_if_not_exists(Conn, Database) ->
    Query = "select 1 from pg_database where datname = '" ++ Database ++ "'",
    case epgsql:squery(Conn, Query) of
        {ok, _, []} -> if_ok(epgsql:squery(Conn, "create database " ++ Database));
        Other -> if_ok(Other)
    end.
...
connection_opts(_Args, {url, DatabaseUrl}) ->
    case uri_string:parse(string:trim(DatabaseUrl)) of
        ...
        Map = #{userinfo := UserPass, host := Host, path := Path} ->
            ...
            {User, Pass} =
                case string:split(UserPassStr, ":") of
                    [[]] -> {"postgres", ""};
                    [U] -> {U, ""};
                    [[], []] -> {"postgres", ""};
                    [U, P] -> {U, P}
            ...
            apply_query_opts(ConnectionOpts, maps:get(query, Map, []))
    end.

apply_query_opts(Opts, []) ->
    {ok, Opts};
apply_query_opts(Opts, "?" ++ QueryString) ->
    apply_ssl_opt(Opts, uri_string:dissect_query(QueryString)).

apply_ssl_opt(Opts, []) ->
    {ok, Opts};
apply_ssl_opt(Opts, QueryList) ->
    case proplists:get_value("ssl", QueryList) of
        undefined -> {ok, Opts};
        [] -> {ok, maps:put(ssl, true, Opts)};
        Value -> {ok, maps:put(ssl, list_to_atom(Value), Opts)}
    end.
